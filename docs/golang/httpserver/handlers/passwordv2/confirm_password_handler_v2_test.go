...
var _ = Describe("The confirm password handler", func() {
	...
	Context("when a valid confirm password V2 request is sent", func() {
		JustBeforeEach(func() {
		...
		It("calls customer auth confirm password V2", func() {
			Expect(saFakeClient.ConfirmPasswordV2CallCount()).To(BeNumerically("==", 1))
			model, headers, _, _, _ := saFakeClient.ConfirmPasswordV2ArgsForCall(0)
	...
	Context("when an invalid request with unmarshallable body is sent to v2 confirm password", func() {
		JustBeforeEach(func() {
			dynamoFakeClient.GetStub = func(t string, k map[string]string, o interface{}, c apm.TransactionContext) error {
				data, _ := json.Marshal(authEntity)
				json.Unmarshal(data, o)
		...
		It("does not call customer auth confirm password", func() {
			Expect(saFakeClient.ConfirmPasswordV2CallCount()).To(Equal(0))
		})
	...
	Context("when a request missing email is sent to v2 confirm password", func() {
		JustBeforeEach(func() {
			dynamoFakeClient.GetStub = func(t string, k map[string]string, o interface{}, c apm.TransactionContext) error {
				data, _ := json.Marshal(authEntity)
				json.Unmarshal(data, o)
