...
var _ = Describe("The forgot password handler", func() {
	var (
	...
	BeforeEach(func() {
		...
		subject = NewForgotPasswordHandler(saFakeClient, dynamoFakeClient, forterFakeClient)
	...
	Context("when a valid request is sent", func() {
		JustBeforeEach(func() {
			...
			requestedPath := "/v1/password/forgot?code=" + code + "&verifier=" + verifier
		...
		It("calls customer auth forgot password", func() {
			Expect(saFakeClient.ForgotPasswordCallCount()).To(BeNumerically("==", 1))
			model, headers, _, _, _ := saFakeClient.ForgotPasswordArgsForCall(0)
	...
	Context("when an invalid request with unmarshallable body is sent ", func() {
		...
		It("does not call customer auth forgot password", func() {
			Expect(saFakeClient.ForgotPasswordCallCount()).To(Equal(0))
		})
